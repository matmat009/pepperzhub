...
/**
 * Single-image picker for a review's photo.
 *
 * Behaviourally identical to the payment-methods QrCodeUpload — same three
 * states the form has to distinguish, which is why removal is its own flag
 * rather than "no file selected":
 ...
 *
 * Only the labels and the empty-state icon differ: this is a customer photo,
 * not a QR code, so it previews at the aspect ratio it will be shown in.
 */
const file = defineModel<File | null>('file', { required: true });
const removed = defineModel<boolean>('removed', { required: true });

const props = defineProps<{
    /** The stored photo, if this review already has one. */
...
<template>
    <div class="grid gap-3">
        ...
        >
            <div
                v-if="shownUrl"
                class="grid w-fit max-w-full place-items-center overflow-hidden rounded-lg border border-sf-serenity-blue/20 bg-white p-1.5 shadow-sm"
            >
                <img
                    :src="shownUrl"
                    alt="Review photo"
                    class="block h-auto max-h-96 w-auto max-w-full rounded-md object-contain"
                />
            </div>
            <div v-else class="grid place-items-center gap-3 text-center">
                ...
                >
                    <ImageIcon
                        class="size-24 max-w-full text-sf-primary-soft"
                    />
                </div>
        ...
        <p class="text-xs text-muted-foreground">
            JPG, PNG or WebP, up to 5MB. Shown to customers on the Reviews page.
        </p>
    </div>
</template>
